...
/// BORU-CALL-9.9: repeated teardown stress test — 75 sequential synthetic
/// calls verifying the five no-leak invariants at stress scale.
///
/// Builds on the 5.3 repeated start/end test (30 iterations); this is the
/// 50-100 iteration stress-scale version. The five invariants:
///
/// 1. no stuck call state — every iteration's call completes and the next
///    call is accepted immediately;
/// 2. no retained connection — the same two endpoints serve all iterations;
/// 3. no continually increasing task count — exact event accounting: each
///    iteration must emit exactly the nine expected events (client: Ringing,
///    Active, Ended; server: Incoming, Active, Ended) and both channels must
///    drain to empty after every hangup. A leaked task would emit strays and
///    the totals would drift from `ITERATIONS * 3`;
/// 4. no dead microphone/camera state — hangup always succeeds and the next
///    call activates (capture-side state is not wedged by the previous stop);
/// 5. no stale Ended event killing the next session — each iteration's Ended
///    carries the current call_id and no leftover Ended survives into the
///    next iteration (channels empty after hangup).
#[tokio::test]
async fn repeated_teardown_stress_75_sequential_calls_no_leaks() {
    ...
    // Exact event accounting: 2 events per side per iteration, no strays.
    let mut client_events_seen = 0usize;
    let mut server_events_seen = 0usize;
    ...
    for iteration in 0..ITERATIONS {
        ...
        // No residual events: a leaked call state or connection would leave
        // strays behind after the terminal Ended, and a stale Ended from a
        // previous session would surface here.
        assert!(
            client_events.try_recv().is_err(),
            "iteration {iteration}: client channel has leftover events"
        );
    ...
    // Exact accounting: 3 events per side per iteration (client: Ringing,
    // Active, Ended; server: Incoming, Active, Ended) and no more.
    assert_eq!(client_events_seen, ITERATIONS * 3);
    assert_eq!(server_events_seen, ITERATIONS * 3);
    assert!(
        client_events.try_recv().is_err(),
        "client channel must be empty after the stress loop"
    );
    assert!(
        server_events.try_recv().is_err(),
        "server channel must be empty after the stress loop"
    );
