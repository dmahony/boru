...
/// Same two-peer gossip flow, but Peer A broadcasts a message signed with
/// deflate compression (`sign_and_encode_compressed`).  Verifies that a real
/// boru instance on the receiving end can verify and inflate it.
#[tokio::test]
async fn test_compressed_signed_message_gossip_flow() -> Result<()> {
    let mut rng = rand::rngs::ChaCha12Rng::seed_from_u64(43);
    ...
    // Wait for peer A to see peer B as a neighbor before broadcasting.
    let deadline_a = tokio::time::Instant::now() + Duration::from_secs(10);
    let b_id = ep_b.id();
    let mut a_saw_b = false;
    while tokio::time::Instant::now() < deadline_a {
    ...
    // Peer A broadcasts a compressed, signed message (repetitive text so
    // compression has real material to work with).
    let msg = Message::Message {
        text: "compressed hello hello hello hello hello hello hello hello".into(),
    };
    let encoded =
        SignedMessage::sign_and_encode_compressed(&sk_a, &msg).expect("sign_and_encode_compressed");
    println!(
        "Peer A broadcasts {} bytes of compressed Message",
        encoded.len()
    );

    // Sanity: compression actually shrank the wire payload.
    let plain = SignedMessage::sign_and_encode(&sk_a, &msg).expect("sign_and_encode");
    assert!(
    ...
    // Check that Peer B received and decoded the compressed message
    let mut received = Vec::new();
    loop {
        match timeout(Duration::from_millis(300), sub_b.try_next()).await {
            Ok(Ok(Some(ev))) => match ev {
                GossipEvent::Received(msg) => {
                    match SignedMessage::verify_and_decode(&msg.content) {
                        ...
                        Err(e) => {
                            println!("  ✗ DECODE ERROR (compressed): {e}");
                        }
                    }
                }
                GossipEvent::NeighborUp(id) => println!("Peer B: NeighborUp {}", id.fmt_short()),
    ...
    assert!(
        !received.is_empty(),
        "Peer B should have received at least one compressed message"
    );
    assert_eq!(received[0].0, sk_a.public(), "From should be Peer A");
    match &received[0].1 {
        Message::Message { text } => assert_eq!(
            text, "compressed hello hello hello hello hello hello hello hello",
            "decoded compressed message text should match"
        ),
    ...
    println!("✓✓ COMPRESSED GOSSIP TEST PASSED ✓✓");
    Ok(())
}
