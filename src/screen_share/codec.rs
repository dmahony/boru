...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum CodecKind {
    /// H.264/AVC via OpenH264 (BSD-2-Clause), the compatibility baseline.
    H264,
...
impl CodecKind {
    /// Wire codec name as negotiated in `Hello.codecs` / `ScreenShareOffer.codecs`
    /// (lowercase, matches the existing "h264" convention).
    pub const fn name(self) -> &'static str {
        match self { Self::H264 => "h264", Self::Av1 => "av1" }
    }

    /// Parse a negotiated codec name case-insensitively. Unknown names map to
    /// `None` so callers can reject or fall back cleanly (H.264 fallback).
...
/// `ScreenShareOffer.codecs` (e.g. "h264" or "av1"). Unknown names are a
/// clean rejection — the caller falls back to H.264 (which is always first in
/// the advertised list) before this is reached.
pub fn create_encoder(
    codec_name: &str,
    config: CodecConfig,
) -> Result<Box<dyn VideoEncoder>, ScreenShareError> {
    match CodecKind::from_name(codec_name) {
        Some(CodecKind::H264) => Ok(Box::new(OpenH264Encoder::new(config)?)),
        Some(CodecKind::Av1) => Ok(Box::new(Av1Encoder::new(config)?)),
