...
/// Register a peer's identity and pre-key in the shared registry using
...
fn register_peer(enc: &mut EncryptionState, peer: PeerId) {
    ...
    enc.registry.insert_identity(&peer, &bundle).unwrap();
    enc.registry.insert_pre_keys(&peer, &[bundle]).unwrap();
}
...
mod integration {
    ...
    fn test_member_add() {
        ...
        bob.receive_message(&group_id, &create_msg)
            .expect("bob receive create");
        ...
        // Alice processes the add control message.
        let _alice_event = alice
    ...
    fn test_member_remove() {
        ...
        let _alice_msg = alice
            .send_message(&group_id, b"secret after removal")
            .expect("alice send after remove");

        let _charlie_result = charlie_state
            .receive_message(&group_id, &_alice_msg)
            .expect("charlie receive after removal");
    }
