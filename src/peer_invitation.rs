...
const STABLE_URI_PREFIX: &str = "boru1:";
...
/// Discovery data that may safely be used to build an invitation.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct SafeDiscoveryInfo {
    /// The local public identity.
    pub peer_id: PublicKey,
    /// User-facing display name.
    pub display_name: String,
    /// Optional content-addressed avatar identifier.
    pub avatar_hash: Option<String>,
    /// Relay/bootstrap hints.
    pub relay_urls: Vec<String>,
    /// Endpoint addresses discovered locally. Never shared by default.
    pub direct_addresses: Vec<String>,
}

/// User-facing summary shown before accepting an invitation.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct InvitationPreview {
    /// Display name from the invitation.
    pub display_name: String,
    /// Truncated public identity.
    pub short_peer_id: String,
    /// Safe avatar identifier, if supplied.
    pub avatar_hash: Option<String>,
    /// Expiry timestamp, if short-lived.
    pub expires_at: Option<i64>,
    /// Relay/bootstrap hints.
    pub relay_urls: Vec<String>,
    /// All connection hints, including explicitly shared direct addresses.
    pub connection_hints: Vec<String>,
    /// Whether direct addresses were included.
    pub includes_direct_addresses: bool,
}

/// Specific errors for pasted or scanned invitation input.
#[derive(Debug, PartialEq, Eq)]
pub enum InvitationInputError {
    /// No non-whitespace input was supplied.
    Empty,
    /// A known URI was supplied but its payload was invalid.
    MalformedUri(String),
    /// The input is neither a supported URI nor a raw payload.
    UnsupportedFormat,
    /// The decoded invitation failed validation.
    Invalid(ValidationError),
}

impl std::fmt::Display for InvitationInputError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::Empty => write!(f, "invitation input is empty"),
            Self::MalformedUri(msg) => write!(f, "malformed invitation URI: {msg}"),
            Self::UnsupportedFormat => write!(f, "unsupported invitation format"),
            Self::Invalid(err) => err.fmt(f),
        }
    }
}

impl std::error::Error for InvitationInputError {}

impl PeerInvitation {
    /// Build an invitation from discovery data, excluding direct addresses
    /// unless the caller explicitly opts in.
    pub fn from_safe_discovery(
        info: &SafeDiscoveryInfo,
        ...
        Self {
            ...
            display_name: info.display_name.trim().to_owned(),
            avatar_hash: info.avatar_hash.clone(),
            relay_urls: info.relay_urls.clone(),
            direct_addresses: if share_direct_addresses {
                info.direct_addresses.clone()
            } else {
                Vec::new()
            },
            friend_request_token: Some(new_pairing_token().to_string()),
            expires_at,
        }
    }

    /// Return a copy with a fresh short-lived pairing token.
    pub fn regenerated(&self) -> Self {
        let mut next = self.clone();
        next.friend_request_token = Some(new_pairing_token().to_string());
        next
    }

    /// Serialize the complete URI, including its scheme and payload.
    pub fn serialize_uri(&self) -> Result<String, EncodeError> {
        self.to_uri()
    }

    /// Create the structured confirmation summary used by frontends.
    pub fn preview(&self) -> InvitationPreview {
        InvitationPreview {
            display_name: self.display_name.clone(),
            short_peer_id: short_peer_id(&self.peer_id),
            avatar_hash: self.avatar_hash.clone(),
            expires_at: self.expires_at,
            relay_urls: self.relay_urls.clone(),
            connection_hints: self
                .relay_urls
                .iter()
                .chain(self.direct_addresses.iter())
                .cloned()
                .collect(),
            includes_direct_addresses: !self.direct_addresses.is_empty(),
        }
    }

    /// Human-readable expiry information for confirmation UIs.
    pub fn expiry_summary(&self, now_unix_secs: i64) -> Option<String> {
        self.expires_at.map(|expires_at| {
            if expires_at <= now_unix_secs {
                "expired".to_string()
            } else {
                format!("expires at UNIX timestamp {expires_at}")
            }
        })
    }

    /// Parse full URI, raw payload, whitespace, and supported legacy forms.
    /// The caller's original input is never modified; normalization is local.
    pub fn parse_input(input: &str) -> Result<Self, InvitationInputError> {
        let normalized = input.split_whitespace().collect::<String>();
        if normalized.is_empty() {
            return Err(InvitationInputError::Empty);
        }
        let payload = if let Some(payload) = normalized.strip_prefix(URI_PREFIX) {
            payload
        } else if let Some(payload) = normalized.strip_prefix("boru-chat://invite/") {
            payload
        } else if normalized.starts_with("boru-chat://") {
            return Err(InvitationInputError::MalformedUri(
                "unknown invitation path".to_string(),
            ));
        } else if normalized.starts_with(STABLE_URI_PREFIX) {
            return Err(InvitationInputError::MalformedUri(
                "stable room invitations are not peer invitations".to_string(),
            ));
        } else {
            normalized.as_str()
        };
        let invitation = Self::decode(payload)
            .map_err(|err| InvitationInputError::MalformedUri(err.to_string()))?;
        invitation
            .validate(None)
            .map_err(InvitationInputError::Invalid)?;
        Ok(invitation)
    }
...
/// Generate a fresh pairing token using the operating system RNG.
pub fn new_pairing_token() -> u64 {
    rand::random()
}

/// Return a short, display-safe representation of a public identity.
pub fn short_peer_id(peer_id: &PublicKey) -> String {
...
/// Parse invitation input without modifying the caller's original string.
pub fn parse_invitation_input(input: &str) -> Result<PeerInvitation, InvitationInputError> {
    PeerInvitation::parse_input(input)
}
...
mod tests {
    ...
    #[test]
    fn safe_discovery_omits_direct_addresses_by_default() {
        let info = SafeDiscoveryInfo {
            peer_id: iroh::SecretKey::generate().public(),
            display_name: " Alice ".into(),
            avatar_hash: Some("avatar:1".into()),
            relay_urls: vec!["relay.example".into()],
            direct_addresses: vec!["192.168.1.5:1234".into()],
        };
        let invite = PeerInvitation::from_safe_discovery(&info, false, Some(i64::MAX));
        assert!(invite.direct_addresses.is_empty());
        assert_eq!(invite.display_name, "Alice");
        assert!(invite.friend_request_token.is_some());
    }

    #[test]
    fn explicit_address_sharing_and_regeneration_work() {
        let info = SafeDiscoveryInfo {
            peer_id: iroh::SecretKey::generate().public(),
            display_name: "Alice".into(),
            avatar_hash: None,
            relay_urls: vec![],
            direct_addresses: vec!["10.0.0.2:9".into()],
        };
        let invite = PeerInvitation::from_safe_discovery(&info, true, Some(i64::MAX));
        assert_eq!(invite.direct_addresses.len(), 1);
        let regenerated = invite.regenerated();
        assert_ne!(
            invite.friend_request_token,
            regenerated.friend_request_token
        );
    }

    #[test]
    fn parse_input_accepts_uri_raw_payload_and_line_breaks() {
        let invite = valid_invitation();
        let uri = invite.to_uri().unwrap();
        let raw = invite.encode().unwrap();
        assert_eq!(
            PeerInvitation::parse_input(&format!("  {}  ", uri)).unwrap(),
            invite
        );
        assert_eq!(PeerInvitation::parse_input(&raw).unwrap(), invite);
        let split = format!("{}\n{}", &uri[..uri.len() / 2], &uri[uri.len() / 2..]);
        assert_eq!(PeerInvitation::parse_input(&split).unwrap(), invite);
    }

    #[test]
    fn parse_input_reports_specific_errors_and_preserves_input() {
        let input = "  boru-chat://unknown/x\n".to_string();
        let original = input.clone();
        assert!(matches!(
            PeerInvitation::parse_input(&input),
            Err(InvitationInputError::MalformedUri(_))
        ));
        assert_eq!(input, original);
        assert_eq!(
            PeerInvitation::parse_input("\n \t").unwrap_err(),
            InvitationInputError::Empty
        );
    }

    #[test]
    fn preview_exposes_safe_confirmation_fields() {
        let mut invite = valid_invitation();
        invite.avatar_hash = Some("avatar:1".into());
        invite.relay_urls = vec!["relay.example".into()];
        invite.expires_at = Some(123);
        let preview = invite.preview();
        assert_eq!(preview.display_name, invite.display_name);
        assert_eq!(preview.avatar_hash, invite.avatar_hash);
        assert_eq!(preview.expires_at, Some(123));
        assert_eq!(preview.relay_urls.len(), 1);
        assert!(!preview.includes_direct_addresses);
    }
}
