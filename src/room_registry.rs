...
pub async fn lookup_registry(
    ...
    let namespace = room_registry_namespace(network);
    let encrypted = backend.lookup(&namespace).await?;
    ...
    for er in encrypted {
        ...
        match record.content::<RoomRegistryEntry>() {
            Ok(entry) if entry.version == ROOM_REGISTRY_CONTENT_VERSION => entries.push(entry),
            _ => {}
        }
    }
    Ok(entries)
