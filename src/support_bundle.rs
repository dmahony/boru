...
pub fn build(input: &SupportBundleInput) -> SupportBundle {
    ...
    if let Some(diagnostics) = &input.diagnostics {
        for event in diagnostics.all_events().iter().rev().take(MAX_WARNINGS) {
            if let Some(warning) = redact_event(event) {
    ...
    if let Some(journal) = &input.journal {
        for entry in journal.all_entries().iter().rev().take(MAX_WARNINGS) {
            if !entry.success {
                warnings.push(RedactedWarning {
                    sequence: Some(entry.sequence),
                    kind: "gui_update_failed".into(),
                    message: "GUI update failed (details redacted)".into(),
                });
            }
        }
    }
    ...
    let mut bundle = SupportBundle {
        ...
        platform: PlatformSummary {
            os: safe_label(&input.os),
            arch: safe_label(&input.arch),
            enabled_features: input
                .enabled_features
                .iter()
                .map(|s| safe_label(s))
                .collect(),
        ...
        endpoint: EndpointSummary {
            short_id: short_id(&input.endpoint_id),
            relay_transport: safe_label(&input.relay_transport),
            connection_paths: input
                .connection_paths
                .iter()
                .map(|s| safe_label(s))
                .collect(),
        ...
        network: NetworkSummary {
            dht_health: safe_label(&input.dht_health),
            active_subscription_count: input.active_subscription_count,
            active_task_count: input.active_task_count,
        },
        storage: StorageSummary {
            schema_version: safe_label(&input.schema_version),
        },
        warnings,
        summary: String::new(),
    };
    bundle.summary = format!(
        ...
        bundle.platform.os,
        bundle.platform.arch,
        bundle.endpoint.short_id,
        bundle.endpoint.relay_transport,
        bundle.network.dht_health,
        bundle.warnings.len()
    );
    bundle
}
...
pub fn export_json(path: &Path, input: &SupportBundleInput) -> Result<()> {
    ...
    std::fs::write(path, bytes)
        .with_context(|| format!("write support bundle to {}", path.display()))
}
fn redact_event(event: &DiagnosticEvent) -> Option<RedactedWarning> {
    let (kind, message) = match &event.kind {
        ...
        DiagnosticEventKind::RoomSubscriptionFailed { .. } => {
            ("subscription_failed", "Room subscription failed")
        }
        DiagnosticEventKind::RoomJoinFailed => ("room_join_failed", "Room join failed"),
        DiagnosticEventKind::CatalogueFetchFailed { .. } => {
            ("catalogue_fetch_failed", "Catalogue fetch failed")
        }
        DiagnosticEventKind::CatalogueSignatureRejected { .. } => (
            "catalogue_signature_rejected",
            "Catalogue signature rejected",
        ),
        DiagnosticEventKind::ProbeTimedOut { .. } => {
            ("probe_timed_out", "Diagnostic probe timed out")
        }
        DiagnosticEventKind::ActionTimedOut { .. } => ("action_timed_out", "GUI action timed out"),
        _ => return None,
    };
    Some(RedactedWarning {
...
fn short_id(value: &str) -> String {
    safe_label(value).chars().take(12).collect()
}
...
mod tests {
    ...
    fn redacts_secrets_messages_files_and_paths() {
        ...
        diagnostics.record(
            None,
            DiagnosticEventKind::Error("AUTH_TOKEN=secret message body /tmp/file".into()),
        );
        let input = SupportBundleInput {
            build_sha: "abc".into(),
            os: "/host/private".into(),
            endpoint_id: "abcdef-secret".into(),
        ...
        assert!(!json.contains("/tmp/file"));
        assert!(json.contains("details redacted"));
    }
    #[test]
    fn hostile_labels_are_path_safe() {
        let bundle = build(&SupportBundleInput {
            endpoint_id: "../../../../secret".into(),
            os: "x\n/y".into(),
            ..Default::default()
        });
        assert_eq!(bundle.endpoint.short_id, ".._.._.._.._");
        assert!(!bundle.platform.os.contains('\n'));
    }
}
