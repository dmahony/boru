//! Focused offline tests for the discovery-service facade.
//!
//! Kept outside `discovery_service.rs` so the production coordinator remains
//! readable while the receive, lifecycle, restart, and shutdown coverage
//! stays close to the public facade API.
...
async fn handle_incoming_unknown_version_all_variants_ignored() {
    ...
    for bytes in [
        ...
        with_protocol_version(
            postcard::to_stdvec(&DiscoveryMessage::peer_advertisement(peer, advertised)).unwrap(),
            99,
        ),
    ] {
        assert_eq!(
...
async fn discovery_advertisement_dials_sender_and_advertised() {
    ...
    let bytes =
        postcard::to_stdvec(&DiscoveryMessage::peer_advertisement(sender_pk, advertised)).unwrap();
...
async fn handle_incoming_room_advertisement_emits_dedicated_event() {
    ...
    let bytes =
        ControlEnvelope::public_room_advertisement(peer, 7, 1_700_000_000, test_advert()).encode();
    let outcome = service.handle_incoming(&bytes, peer);
    assert_eq!(outcome, IncomingOutcome::ControlMessage);
...
async fn handle_incoming_unsigned_room_advertisement_is_untrusted() {
    ...
    let bytes =
        ControlEnvelope::public_room_advertisement(peer, 14, 1_700_000_000, test_advert()).encode();
    let outcome = service.handle_incoming(&bytes, peer);
    assert_eq!(outcome, IncomingOutcome::ControlMessage);
...
async fn handle_incoming_advertisement_rejection_does_not_affect_gossip() {
    ...
    let bad = ControlEnvelope::public_room_advertisement(peer, 15, 1_700_000_000, advert).encode();
    assert_eq!(
...
async fn handle_incoming_verified_room_withdrawal_emits_dedicated_event() {
    ...
    let bytes =
        ControlEnvelope::public_room_withdrawal(owner, 21, 1_700_000_000, withdrawal).encode();
...
async fn handle_incoming_tampered_room_withdrawal_rejected() {
    ...
    let bytes =
        ControlEnvelope::public_room_withdrawal(owner, 22, 1_700_000_000, withdrawal).encode();
    let outcome = service.handle_incoming(&bytes, owner);
    assert_eq!(
        outcome,
...
async fn announce_room_withdrawal_broadcasts_signed_envelope() {
    ...
    match ControlEnvelope::decode(&bytes).expect("control envelope decodes") {
        ControlPlaneDecode::Message(env) => {
            assert_eq!(env.sender_node_id, local);
            assert_eq!(env.message_type, ControlMessageType::PublicRoomWithdrawal);
...
async fn handle_incoming_room_advertisement_populates_directory() {
    ...
    let bytes =
        ControlEnvelope::public_room_advertisement(peer, 7, 1_700_000_000, advert.clone()).encode();
    let outcome = service.handle_incoming(&bytes, peer);
    assert_eq!(outcome, IncomingOutcome::ControlMessage);
    ...
    let entry = guard
        .get(&advert.room_id)
        .expect("room cached by stable room_id");
...
async fn handle_incoming_duplicate_advertisements_merge() {
    ...
    let refresh =
        ControlEnvelope::public_room_advertisement(peer, 8, 1_700_000_060, advert.clone()).encode();
    assert_eq!(
        service.handle_incoming(&refresh, peer),
        IncomingOutcome::ControlMessage
    );
...
async fn handle_incoming_identical_content_different_publisher_no_second_event() {
    ...
    let first =
        ControlEnvelope::public_room_advertisement(peer_a, 7, 1_700_000_000, advert.clone())
            .encode();
    assert_eq!(
        service.handle_incoming(&first, peer_a),
        IncomingOutcome::ControlMessage
    );
    let event = tokio::time::timeout(Duration::from_secs(5), events.recv())
        .await
    ...
    let second =
        ControlEnvelope::public_room_advertisement(peer_b, 1, 1_700_000_100, same_content).encode();
    assert_eq!(
...
async fn handle_incoming_conflicting_advertisements_merge_with_conflict_flag() {
    ...
    assert_eq!(
        guard.len(),
        1,
        "conflicting ads still collapse to one entry"
    );
...
async fn handle_incoming_verified_withdrawal_removes_directory_entry() {
    ...
    let bytes = ControlEnvelope::public_room_advertisement(owner, 7, 1_700_000_000, advert.clone())
        .encode();
    assert_eq!(
...
async fn handle_incoming_non_authoritative_withdrawal_keeps_directory_entry() {
    ...
    assert_eq!(
        guard.len(),
        1,
        "non-authoritative withdrawal cannot remove the room"
    );
    assert_eq!(service.peer_count(), 0);
}
...
async fn directory_counters_invalid_signature_increments_rejected() {
    ...
    let mut advert = test_advert();
    advert.sign(&test_secret_key(0xCC));
    let bytes = ControlEnvelope::public_room_advertisement(peer, 7, 1_700_000_000, advert).encode();
    assert_eq!(
        service.handle_incoming(&bytes, peer),
        IncomingOutcome::AdvertisementAuthRejected
    );
...
async fn directory_counters_duplicate_increments_deduplicated() {
    ...
    assert_eq!(
        snap.advertisements_accepted, 1,
        "only the first insert is accepted"
    );
...
async fn directory_counters_rate_limited_increments_on_flood() {
    ...
    for seq in 0..60u64 {
        ...
        advert.sign(&test_secret_key(0xBB));
        let bytes =
            ControlEnvelope::public_room_advertisement(peer, seq, 1_700_000_000, advert).encode();
    ...
    let bytes = ControlEnvelope::public_room_advertisement(peer, 60, 1_700_000_000, extra).encode();
    assert_eq!(
        service.handle_incoming(&bytes, peer),
    ...
    assert_eq!(
        snap.advertisements_rejected, 0,
...
async fn directory_expiry_sweep_evicts_expired_entries() {
    let local = test_key(0xAA);
    let service = test_service(local).with_directory_sweep_interval(Duration::from_millis(50));
...
async fn directory_diagnostics_distinguishes_room_outcomes() {
    ...
    let bytes = ControlEnvelope::public_room_advertisement(owner, 7, 1_700_000_000, expired_advert)
        .encode();
    assert_eq!(
        service.handle_incoming(&bytes, owner),
        IncomingOutcome::ControlMessage
    ...
    let bytes = ControlEnvelope::public_room_advertisement(owner, 8, 1_700_000_000, bad).encode();
    assert_eq!(
        service.handle_incoming(&bytes, owner),
        IncomingOutcome::AdvertisementAuthRejected
    );
    ...
    let bytes =
        ControlEnvelope::public_room_advertisement(owner, 9, 1_700_000_000, live_advert).encode();
    assert_eq!(
        service.handle_incoming(&bytes, owner),
        IncomingOutcome::ControlMessage
    );
...
async fn announce_capabilities_broadcasts_capabilities_envelope() {
    ...
    match ControlEnvelope::decode(&bytes).expect("control envelope decodes") {
        ControlPlaneDecode::Message(env) => {
            ...
            let crate::control_plane::message::ControlPayload::Capabilities(payload) = &env.payload
            else {
...
async fn announce_extensions_broadcasts_extensions_envelope() {
    ...
    match ControlEnvelope::decode(&bytes).expect("control envelope decodes") {
        ControlPlaneDecode::Message(env) => {
            ...
            let crate::control_plane::message::ControlPayload::Extensions(payload) = &env.payload
            else {
...
async fn extensions_expire_with_presence_ttl() {
    ...
    let outcome = service.handle_incoming(&control_extensions(peer, 1, sample_extensions()), peer);
    assert_eq!(outcome, IncomingOutcome::ControlMessage);
    assert!(
