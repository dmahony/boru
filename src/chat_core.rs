...
pub enum Message {
    ...
    FileShare {
        ...
        size: u64,
        /// Optional video thumbnail (JPEG bytes) generated by the
        /// sender for movie/video files.  Populated for .mov, .mp4,
        /// .avi, .mkv, .webm and similar video extensions when
        /// ffmpeg is available on the sender's machine.
        #[serde(default)]
                    ...
                    if from != cb.local_public() {
                        ...
                        if cb.is_friend(&from) || cb.accepts_group_peer(topic, &from) {
                            ...
                            if !is_muted {
                                ...
                                cb.set_pending_file(name, ticket, size, thumbnail);
                            }
                        }
                    }
                }
