...
fn shared_gif_unknown_provider_round_trips_in_message() {
    ...
    match decoded {
        Message::SharedGif { gif } => {
            ...
            assert_eq!(
                gif.format,
                crate::gif_provider::GifMediaFormat::AnimatedWebP
            );
        }
...
fn signed_message_has_sender_generated_id_and_rejects_unknown_compression() {
    let key = SecretKey::generate();
    let encoded =
        SignedMessage::sign_and_encode(&key, &Message::Message { text: "id".into() }).unwrap();
    let envelope: SignedMessage = postcard::from_bytes(&encoded).unwrap();
    assert_ne!(envelope.message_id.as_slice(), [0; 32]);
    assert_eq!(
        SignedMessage::verify_and_decode_with_id(&encoded)
            .unwrap()
            .3,
...
fn old_wire_format_file_share_decodes_to_single_file_defaults() {
    ...
    match decoded {
        Message::FileShare {
            ...
            assert_eq!(
                collection_hash, None,
                "legacy payload must be a single-file share"
            );
