...
impl RoomAdapter {
    ...
    pub fn new(config: TestControlConfig) -> Self {
        Self { config, next_capability: 1, rooms: HashMap::new() }
    }
    ...
    pub fn create_room(
        ...
        if run_id.is_empty() || workflow_id.is_empty() || marker.as_ref().is_empty() {
            return Err(E2eError::new(E2eErrorCode::InvalidState, "room identifiers must not be empty"));
        }
        if self.rooms.contains_key(&marker) {
            return Err(E2eError::new(E2eErrorCode::InvalidState, "room already exists"));
        }
        ...
        self.rooms.insert(marker.clone(), RoomRecord {
            room_id_hash,
    ...
    pub fn join_room(&mut self, node_alias: NodeAlias, handle: &RoomJoinHandle) -> Result<RoomSnapshot, E2eError> {
        self.require_mutation()?;
        self.validate_handle(handle)?;
        let room = self.room_mut(&handle.marker)?;
        if matches!(room.state, RoomState::Joining | RoomState::Leaving) {
            return Err(E2eError::new(E2eErrorCode::InvalidState, "room transition already in progress"));
        }
        room.members.insert(node_alias);
        room.state = RoomState::Joined;
        room.last_transition = Some("joined".to_owned());
        room.last_error = None;
        self.snapshot(&handle.marker)
    }

    /// Leave a room through the normal lifecycle path.
    pub fn leave_room(&mut self, node_alias: &NodeAlias, marker: &RoomMarker) -> Result<RoomSnapshot, E2eError> {
        self.require_mutation()?;
    ...
    pub fn rejoin_room(&mut self, node_alias: NodeAlias, handle: &RoomJoinHandle) -> Result<RoomSnapshot, E2eError> {
        self.join_room(node_alias, handle)
    }
    ...
    fn require_mutation(&self) -> Result<(), E2eError> {
        if self.config.allows_mutation() { Ok(()) } else { Err(E2eError::new(E2eErrorCode::DisabledAction, "room test actions are disabled")) }
    }

    fn validate_handle(&self, handle: &RoomJoinHandle) -> Result<(), E2eError> {
        if Instant::now() >= handle.expires_at {
            return Err(E2eError::new(E2eErrorCode::ExpiredJoinReference, "join reference expired"));
        }
        let room = self.rooms.get(&handle.marker).ok_or_else(|| not_found("room"))?;
        if room.capability != handle.capability {
            return Err(E2eError::new(E2eErrorCode::InvalidJoinReference, "invalid join reference"));
        }
        Ok(())
    }
    ...
    fn handle(&self, marker: RoomMarker, capability: u64) -> RoomJoinHandle {
        RoomJoinHandle { marker, capability, expires_at: Instant::now() + JOIN_HANDLE_TTL }
    }
}

fn stable_room_hash(run_id: &str, workflow_id: &str, marker: &str) -> String {
    blake3::hash(format!("boru-e2e-room/v1\0{run_id}\0{workflow_id}\0{marker}").as_bytes()).to_hex().to_string()
}
...
mod tests {
    ...
    fn adapter() -> RoomAdapter {
        RoomAdapter::new(TestControlConfig { enabled: true, ..Default::default() })
    }
    ...
    fn create_join_leave_rejoin_converges() {
        ...
        let (created, handle) = adapter.create_room("run-1", "node-a".into(), "workflow-1", marker.clone()).unwrap();
    ...
    fn invalid_and_expired_handles_are_structured() {
        let mut adapter = adapter();
        let marker: RoomMarker = "room-1".into();
        let (_, handle) = adapter.create_room("run-1", "node-a".into(), "workflow-1", marker).unwrap();
        let invalid = RoomJoinHandle { capability: handle.capability + 1, ..handle.clone() };
        assert_eq!(adapter.join_room("node-b".into(), &invalid).unwrap_err().code, E2eErrorCode::InvalidJoinReference);
        let expired = RoomJoinHandle { expires_at: Instant::now() - Duration::from_secs(1), ..handle };
        assert_eq!(adapter.join_room("node-b".into(), &expired).unwrap_err().code, E2eErrorCode::ExpiredJoinReference);
    }
