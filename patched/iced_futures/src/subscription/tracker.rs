...
impl Tracker {
            ...
            + Clone,
    {
        ...
        let tracker = self as *const Self as usize;
        ...
        for recipe in recipes {
            ...
            eprintln!(
                "ICED_TRACKER tracker={tracker:x} recipe={id:016x} existed={existed}"
            );
            ...
            eprintln!(
                "ICED_TRACKER_SPAWN tracker={tracker:x} recipe={id:016x}"
            );
                ...
                loop {
                    ...
                    match select.await {
                        futures::future::Either::Left(_) => {
                            eprintln!(
                                "ICED_TRACKER_CANCEL tracker={tracker:x} recipe={id:016x}"
                            );
                            break;
                        }
                        futures::future::Either::Right((None, _)) => {
                            eprintln!(
                                "ICED_TRACKER_STREAM_ENDED tracker={tracker:x} recipe={id:016x} — stream terminated while recipe stays registered (iced will NOT restart it)"
                            );
                            break;
                        }
        ...
        self.subscriptions.retain(|id, _| {
            ...
            if !keep {
                eprintln!(
                    "ICED_TRACKER_REMOVE tracker={tracker:x} recipe={id:016x}"
                );
            }
