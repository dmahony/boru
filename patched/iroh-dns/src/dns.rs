...
///
/// Increased from 3s to 5s to accommodate slower DNS servers (e.g. on
/// congested corporate VPNs or when Windows 11 DoH causes UDP fallback
/// retries through the hickory-resolver).
...
impl HickoryResolver {
    ...
    fn build_resolver(builder: &Builder) -> TokioResolver {
        ...
        // ── DoH fallback nameservers ─────────────────────────────────
        // Windows 11 configures DNS over HTTPS (DoH) by default for
        // compatible servers, but hickory-resolver sends plain UDP queries
        // which are silently dropped.  Add well-known public DoH resolvers
        // as a fallback so that DNS resolution still works when system
        // nameservers only accept HTTPS DNS.
        //
        // These are added *after* the system nameservers so hickory tries
        // the system ones first (faster, local) but falls back to DoH when
        // they time out.
        #[cfg(with_crypto_provider)]
        {
            let doh_servers: [(IpAddr, Arc<str>); 4] = [
                (IpAddr::V4(Ipv4Addr::new(1, 1, 1, 1)), Arc::from("cloudflare-dns.com")),
                (IpAddr::V4(Ipv4Addr::new(1, 0, 0, 1)), Arc::from("cloudflare-dns.com")),
                (IpAddr::V4(Ipv4Addr::new(8, 8, 8, 8)), Arc::from("dns.google")),
                (IpAddr::V4(Ipv4Addr::new(8, 8, 4, 4)), Arc::from("dns.google")),
            ];
            for (ip, hostname) in &doh_servers {
                let nameserver =
                    hickory_resolver::config::NameServerConfig::https(*ip, hostname.clone(), None);
                config.add_name_server(nameserver);
            }
        }
