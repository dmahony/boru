...
pub(crate) unsafe fn decode<T: Copy, C: CMsgHdr>(cmsg: &impl CMsgHdr) -> T {
    // PATCHED (boru, Termux/musl): the kernel writes cmsg payloads aligned to
    // the kernel's own `cmsghdr` (size_t cmsg_len → 8-aligned on LP64), which
    // can be MORE aligned than the libc crate's struct on musl (socklen_t
    // cmsg_len → struct align 4). Comparing type alignments is therefore wrong
    // on musl: decoding SCM_TIMESTAMPNS (timespec, align 8) with a 4-aligned
    // `libc::cmsghdr` panics even though the actual payload pointer is
    // 8-aligned. Check the real pointer instead (matches quinn upstream).
    assert_eq!(
        (cmsg.cmsg_data() as usize) % align_of::<T>(),
        0,
        "unaligned cmsg payload for T={}, C={}",
        type_name::<T>(),
        type_name::<C>(),
    );
