...
#
# Builds the project (library + the `boru` GUI binary), then creates a
# `boru-chat` symlink pointing to the `boru` binary so users who
...
# Build the library and the boru GUI binary (the main application)
cargo build $CARGO_FLAGS $FEATURES --bin boru

# Determine the binary path
EXAMPLE_BIN="target/$TARGET_DIR/boru"

if [ ! -f "$EXAMPLE_BIN" ]; then
    echo "error: expected binary not found at $EXAMPLE_BIN" >&2
    exit 1
fi

# Create backward-compatible `boru-chat` symlink
if [ ! -L "target/$TARGET_DIR/boru-chat" ]; then
    ln -s "boru" "target/$TARGET_DIR/boru-chat"
    echo "→ Created backward-compatible symlink: target/$TARGET_DIR/boru-chat → boru"
else
...
echo "  Application:    target/$TARGET_DIR/boru"
echo "  Legacy alias:   target/$TARGET_DIR/boru-chat  (symlink, deprecated)"
echo ""
echo "  Run:  cargo run --features gui"
