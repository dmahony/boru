#!/usr/bin/env bash
# Capture UI-HOME-03 dashboard card foundation evidence on the HOME screen
# (Screen::ChatList).
#
# Usage: scripts/ui_home03_card_evidence.sh <before|after>
#
...
#
# Output: docs/ui-redesign/evidence/t_7595a388/t_7595a388_home_<w>x<h>_<phase>.png
set -euo pipefail
...
for spec in '1600 900' '1280 800' '1024 720'; do
    ...
    data=$(mktemp -d "${TMPDIR:-/tmp}/boru-home03.XXXXXX")
    Xvfb ":$display" -screen 0 "${w}x${h}x24" -nolisten tcp >/tmp/boru-home03-xvfb-$w-$PHASE.log 2>&1 & xv=$!
    sleep 0.5
    DISPLAY=":$display" "$BIN" --data-dir "$data" --no-dht --no-relay --name "UI-HOME-03 $w" \
        >/tmp/boru-home03-app-$w-$PHASE.log 2>&1 & app=$!
    win=''
