...
impl Connection {
    ...
    fn on_loss_detection_timeout(&mut self, now: Instant, path_id: PathId) {
        ...
        let Some((_, space)) = self.pto_time_and_space(now, path_id) else {
            // The PTO timer expired but there is no space with in-flight packets and the
            // handshake is complete (or there is no anti-deadlock case to handle). This is a
            // benign race: the timer was set when in-flight packets existed, but all were
            // acknowledged before the timer fired. Re-arm/stop the timer so it tracks the
            // current state correctly.
            trace!(%path_id, "PTO expired while unset, re-evaluating timer");
            self.set_loss_detection_timer(now, path_id);
            return;
        };
        trace!(
