...
/// is `FillPortion(9)` of a two-column row with a 24 px gap, so the card
/// gets 9/14 of `content_width − SPACE_24`.
///
...
pub fn status_card_content_width(content_width: f32) -> f32 {
    if content_width < HOME_TWO_COL_CONTENT {
        ...
        (content_width - SPACE_24) * 9.0 / 14.0
    }
}
