...
use iroh::{
    address_lookup::memory::MemoryLookup, endpoint::presets, Endpoint, PublicKey,
...
impl AppState {
    fn push_system(&mut self, text: String) {
        self.messages.push(ChatLine { kind: ChatLineKind::System, text });
    }
...
fn main() -> Result<()> {
    ...
    let (
        ...
        let ticket = Ticket { topic, peers: vec![local_peer_addr] };
    ...
    .theme(|state: &AppState| if state.dark_mode { Theme::Dark } else { Theme::Light })
...
fn view(state: &AppState) -> Element<'_, AppMessage, Theme, iced::Renderer> {
    ...
    let status_panel = container(column![text(status_text).size(13)].spacing(2))
        .padding(8)
        .style(move |_| {
            let bg = if state.dark_mode { Color::from_rgb(0.18, 0.18, 0.20) } else { Color::from_rgb(0.93, 0.93, 0.96) };
            container::Style::default()
                .background(bg)
                .border(border::Border::default().width(1).color(Color::from_rgb(0.6, 0.6, 0.6)))
        });

    let log_col = state.messages.iter().fold(
        ...
        |col, line| {
            ...
            col.push(text(&line.text).color(color).size(14).wrapping(Wrapping::Word))
        },
    );
    ...
    composer_children.push(
        row![input_row, toggler(state.dark_mode).label("Dark").on_toggle(AppMessage::ToggleDark)]
            .spacing(16)
    ...
    .padding(8)
    .style(move |_| {
        let bg = if state.dark_mode { Color::from_rgb(0.16, 0.16, 0.18) } else { Color::from_rgb(0.95, 0.95, 0.97) };
        container::Style::default()
            .background(bg)
            .border(border::Border::default().width(1).color(Color::from_rgb(0.6, 0.6, 0.6)))
    });
    ...
    container(content)
        ...
        .height(Length::Fill)
        .style(move |_| {
            let bg = if state.dark_mode { Color::from_rgb(0.12, 0.12, 0.14) } else { Color::from_rgb(0.90, 0.90, 0.93) };
    ...
    match event {
        NetEvent::Message { from, message } => match message {
            ...
            Message::Message { text } => {
                if from == state.local_public { return; }
...
fn handle_send(state: &mut AppState) {
    ...
    if let Some(path) = trimmed.strip_prefix("/send ") {
        ...
        let abs_path = match std::path::absolute(&PathBuf::from(&path)) {
            Ok(p) => p,
            Err(e) => { state.push_system(format!("Failed to resolve path: {e}")); return; }
        };
        if !abs_path.exists() {
            state.push_system(format!("File not found: {path}")); return;
        }
        let filename = match abs_path.file_name().map(|s| s.to_string_lossy()) {
            Some(n) => n.to_string(),
            None => { state.push_system("Invalid file path.".into()); return; }
        };
        state.push_system(format!("Hashing file: {filename}..."));
        let rt = state.runtime_handle.clone();
        let tag = rt.block_on(async {
            state.blob_store.blobs().add_path(abs_path).await.unwrap()
        });
        ...
        if let Ok(encoded) = SignedMessage::sign_and_encode(
            &state.secret_key,
            &Message::FileShare { name: filename.clone(), ticket: ticket_str.clone() },
        ) {
    ...
    if trimmed == "/download" { handle_download(state); return; }
    if trimmed == "/help" {
        state.push_system(
            "Commands:  /send <path> — share a file  |  /download — fetch pending file  |  /help — this help".into(),
        );
        return;
    }

    let msg = Message::Message { text: trimmed.clone() };
...
fn handle_download(state: &mut AppState) {
    let (filename, ticket_str) = match state.pending_file.clone() {
        Some(p) => p,
        None => { state.push_system("No pending file to download.".into()); return; }
    };
    let ticket: BlobTicket = match ticket_str.parse() {
        Ok(t) => t,
        Err(e) => { state.push_system(format!("Failed to parse blob ticket: {e}")); return; }
    };
        ...
        match downloader.download(ticket.hash(), Some(peer_id)).await {
            Ok(_) => {
                ...
                match state.blob_store.blobs().export(ticket.hash(), dest).await {
                    Ok(_) => true,
                    Err(e) => { state.push_system(format!("Export failed: {e}")); false }
                }
            }
            Err(e) => { state.push_system(format!("Download failed: {e}")); false }
        }
    });
