...
1. **text_muted contrast** — **RESOLVED by t_b2ac1e1a** (see
   `UI-HOME-followup-text-muted-contrast.md`): the QA row above measured the
   stale DESIGN_SYSTEM.md hex. The live token in `design_tokens.rs` at this
   commit was already `#64706A` (darkened in `04a3a7fe`), which measures
