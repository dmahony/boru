...
| 5 | The Discover Rooms UI is separate from the user's joined conversation list. | `Screen::Discover` is a distinct screen (`app.rs:3113-3114`), rendered by `view_discover_content` (`src/bin/boru/app/discover.rs:948-1055`), separate from the CHATS sidebar which lists only actual conversations; opening the directory never changes membership (PDF Task 5.1); empty state explains rooms appear when discovered. Matrix scenario 5. | PASS |
