...
- **opus** — links against `libopus` via pkg-config (`libopus-dev` on
  Debian/Ubuntu); if it is not installed, the `opus-sys` build script
  falls back to compiling the vendored Opus source with `configure` +
  `make` (autotools).
...
pkg-config --exists opus && echo opus-OK   # optional: vendored autotools fallback
```
